...
from .bbox_tool import inter_bbox_1to1
...
class ImageOverScanWrapper:
    ...
    def get(self, yx_start, yx_end, pad_value: Union[float, int, Iterable]=0):
        ...
        # 生成输出图像缓存
        out_im_hwc = [yx_end[0]-yx_start[0], yx_end[1]-yx_start[1], im.shape[-1]]
        out_im = np.full(out_im_hwc, pad_value, dtype=im.dtype)

        src_bbox = np.int32([0, 0, im.shape[0], im.shape[1]])
        dst_bbox = np.int32([*yx_start, *yx_end])
        in_bbox = inter_bbox_1to1(src_bbox, dst_bbox)

        # 如果区域完全没有覆盖原图，直接返回空区域
        if in_bbox is None:
            return out_im

        # 获得输出图的写入坐标
        dst_writer_bbox = in_bbox - np.tile(dst_bbox[:2], 2)

        out_im[dst_writer_bbox[0]: dst_writer_bbox[2], dst_writer_bbox[1]: dst_writer_bbox[3]] = \
            im[in_bbox[0]: in_bbox[2], in_bbox[1]: in_bbox[3]]
